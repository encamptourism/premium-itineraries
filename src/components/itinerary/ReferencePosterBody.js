...
export default function ReferencePosterBody({ itinerary, onOpenEnquiry }) {
  ...
  const [isYtStarted, setIsYtStarted] = useState(false);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 items-start">
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="rounded-2xl p-4 sm:p-5 border border-[#e2d8c3] space-y-3 font-poppins bg-[#fbf9f4]">
              ...
              {/* Full Video Main Player (Thumbnail preview with Play button, fills 100% full width, YouTube controls enabled) */}
              <div
                ref={ytContainerRef}
                className="relative w-full aspect-video rounded-2xl overflow-hidden border border-[#e2d8c3] shadow-md bg-stone-900 group"
              >
                {ytVideoId ? (
                  isYtStarted ? (
                    <div className="relative w-full h-full overflow-hidden rounded-2xl bg-black">
                      <iframe
                        key={`${ytVideoId}-${activeYtIndex}`}
                        src={`https://www.youtube.com/embed/${ytVideoId}?autoplay=1&mute=0&controls=1&rel=0&modestbranding=1&playsinline=1`}
                        title={ytVideoTitle}
                    ...
                    >
                      <Image
                        ...
                        className="object-cover group-hover/thumb:scale-105 transition-transform duration-500 rounded-2xl"
                        priority
                      />

                      {/* Dark overlay gradient for contrast */}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/40 rounded-2xl flex flex-col justify-between p-3.5 sm:p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-white text-[11px] font-bold bg-black/60 px-3 py-1 rounded-full backdrop-blur-xs border border-white/20">
                            Tap to Play
                          </span>
                        </div>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-[#f4c430] text-[#1b3d2f] flex items-center justify-center pl-1 shadow-2xl group-hover/thumb:scale-110 transition-transform duration-300 ring-4 ring-white/40">
                            <Play className="w-7 h-7 sm:w-8 sm:h-8 fill-current" />
                          </div>
                          <span className="mt-3 text-white text-xs sm:text-sm font-bold tracking-wide drop-shadow-md">
                            Play {destinationName} Experience
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {shortsToRender.slice(0, 4).map((short, i) => (
                    <div
                      ...
                      className="relative aspect-square rounded-xl sm:rounded-2xl overflow-hidden border border-[#e2d8c3] group cursor-pointer block shadow-xs bg-[#faf7f0]"
                    >
                      <Image
                        src={short.thumbnail}
                        alt={short.title}
                        ...
                        className="object-cover scale-[2.3] group-hover:scale-[2.4] transition-transform duration-300"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/15 to-transparent flex flex-col items-center justify-between p-1.5 text-center pointer-events-none">
      ...
      {activeReelModal && (
        ...
        >
          ...
          >
            ...
            <div className="relative w-full aspect-[9/16] bg-[#faf7f0] overflow-hidden">
              {activeReelModal.videoId ? (
                <iframe
                  src={`https://www.youtube.com/embed/${activeReelModal.videoId}?autoplay=1&playsinline=1&rel=0&controls=0&modestbranding=1&iv_load_policy=3&disablekb=1`}
                  title={activeReelModal.title}
                  className="w-[135%] h-[135%] -translate-x-[17.5%] -translate-y-[17.5%] border-0 transform scale-[1.32] origin-center object-cover"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                />
              ) : (
